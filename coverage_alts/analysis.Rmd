...
datatable(bed_coverage, rownames = FALSE, filter="top", options = list(pageLength = 10, scrollX=T, scrollY=T), caption = "Coverage of alt regions")

cutoff <- 10
bed_coverage_intermediate <- bed_coverage %>%
  dplyr::mutate(across(5:ncol(bed_coverage), ~replace(., . >= cutoff, NA)))
bed_ccoverage_filtered <- bed_coverage_intermediate[complete.cases(bed_coverage_intermediate[ , 5:ncol(bed_coverage_intermediate)]),]
# display filtered data in a table
datatable(bed_ccoverage_filtered, rownames = FALSE, filter="top", options = list(pageLength = 10, scrollX=T, scrollY=T), caption = "Alt rgions with coverage less than the cutoff across tested samples")
```
