...
# outputs df with pos + annotation for each variant of that chromosome
filter_chr <- function(chrom, vcf) {
  vcf_chr_pos_ann <- data.frame(chr = getCHROM(vcf),
                                pos = getPOS(vcf),
                                ann = extract.info(vcf, "ANN"),
                                stringsAsFactors = FALSE)
  return(filter(vcf_chr_pos_ann, chr == chrom))
}
```
...
# Take a df with chr-pos-ann (from filter_chr) and a bin vec (from bin_chr)
# Return a df where col1 = bin_num, col2 = number of mutations in that bin
# Try it out for chr1 first.

chr1_pos_ann <- filter_chr(chr_names[1], vcf) # chr-pos-ann for chr1 ** this takes a couple seconds for chr1
head(chr1_pos_ann, 1)
chr1_bins <- bin_chr(chr_lengths, chr_names[1], bin_size) # 40Mb bins for chr1
(mut_in_chr1_bins <- count_mut_per_bin(chr1_pos_ann, chr1_bins))

# Does the output make sense?
region_ann <- unname(sapply(chr1_pos_ann$ann, function(x) {
  strsplit(x, "\\|")[[1]][2]
}))

table(region_ann) # yep, 14 mutations with those region_ann
...
count_mut_per_bin <- function(chr_pos_ann, bin_vec) {
  ...
  region_ann <- sapply(chr_pos_ann$ann, function(x){
    y <- strsplit(x, "\\|")[[1]][2] # this selects the second element from the split string 'synonymous_variant', 'missense_variant'...
  })
  region_ann <- unname(region_ann) # this is an unnamed character vector of 'synonymous_variant', 'missense_variant'...
  dfs_list <- list()
  # For each element in the vector, do something. Whenever you say something
  # like that in R, you should immediately think that R is already 'vectorised'. See e.g. http://www.noamross.net/blog/2014/4/16/vectorization-in-r--why.html
  # You don't need a for loop to iterate over an atomic vector. Just use the vector as is, and check if it is equal to those strings.
  # e.g. try region_ann %in% c("foo", "bar", "baz"). This will probably simplify your life. Or not. Just try it out and see. At least you'll get to see how
  # R actually works ;-)
  for (i in 1:length(region_ann)) {
    #Filter annotations of interest 
    # Q: why are we interested in only these annotations?
    # So if your element is of interest..
    if (region_ann[i] == "frameshift_variant" | 
        region_ann[i] == "missense_variant" | 
        region_ann[i] == "missense_variant&splice_region_variant") {
      # create a df with pos and findInterval
      # e.g. df with '1801277', '1' columns
      df <- data.frame(chr_pos_ann[i, 2], findInterval(chr_pos_ann[i, 2], bin_vec))
      # then chuck that into the list
      dfs_list[[i]] <- df
    }
  }
  # So up to this point you have a list with n dfs with 1 row.
  
  #check if the annotation used for filtering exists in the vcf - i.e. there are rows in the combined df
  if (length(dfs_list) != 0) {
    #Bind rows together
    combined <- bind_rows(dfs_list) # ok, so this will give you a df of pos-findInterval with n rows
    tab <- table(combined[, 2]) # table of the findInterval thingy
  ...
  #df 
  # I'm not sure why you've commented df out. A function needs to return something. 
  # By coincidence I think it returns df since the last expression in your if-else
  # is df. Better be clearer here.
}
```
...
# So loop over the 24 chromosomes
for (i in 1:length(chr_names)) {
  # count_mut_per_bin
  #check if there exists any mutation with the specified annotation, in the vcf. 
  # Can you see that you're using the same function 3 times here? Think about
  # how you could store the results of it in one variable, then check the if,
  # then take its mean/sum.
...
if (all(sapply(results2, function(x) is.null(x))) != TRUE) {
  names(results2) <- paste("chr", chr_names, sep = "") # check out ?paste0
  str(results2)
