...
file_path_organoid <- "path/to/Avner_Birinapant_Organoid_filtered_wo_rep_CPM_TMM_filtered_log_annot.txt"
file_path_tissue <- "path/to//Avner_Birinapant_Tissue_WGS_matched_filtered_wo_rep_CPM_TMM_filtered_log_annot.txt"
...
gene_expression_organoid <- data_organoid[data_organoid$X == "MLKL", ] %>%
     subset(select=-X)
gene_expression_tissue <- data_tissue[data_tissue$X == "MLKL", ] %>%
     subset(select=-X)
...
missing_columns_organoid <- setdiff(all_columns, names(gene_expression_organoid))
gene_expression_organoid[missing_columns_organoid] <- NA

# Add missing columns to gene_expression_tissue with NA values
missing_columns_tissue <- setdiff(all_columns, names(gene_expression_tissue))
...
p <- plot_ly(data = combined_data_long, x = ~variable, y = ~value, color = ~source, type = "bar") %>%
  layout(title = "Expression of Genes",
         yaxis = list(title = "Expression Value"),
         xaxis = list(title = "Sample Type"))

htmlwidgets::saveWidget(as_widget(p), "path/to/Avner RNAseq/expression_MLKL.html")
